...
describe('Settings Store', () => {
  ...
  it('sets API endpoint and persists to localStorage', () => {
    const store = useSettingsStore()
  ...
  it('normalizes API endpoint by removing trailing slashes', () => {
    const store = useSettingsStore()
    store.setApiEndpoint('http://example.com:4434///')

    expect(store.apiEndpoint).toBe('http://example.com:4434')
  })
  ...
  it('resets API endpoint to default', () => {
    ...
    expect(store.apiEndpoint).toBe('http://custom.example.com:4434')
  ...
  it('computes isConfigured correctly', () => {
    const store = useSettingsStore()
    expect(store.isConfigured).toBe(true)

    store.setApiEndpoint('')
